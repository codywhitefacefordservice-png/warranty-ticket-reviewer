...
function createStore({ name, city, slug, active = true, plan = "pilot" }) {
  ...
  const store = { id: genId("st_"), name: String(name || "").trim() || "Store", city: String(city || "").trim(), slug: s, logoKey: null, logoType: "image/png", logoVer: 0, active: !!active, plan, createdAt: new Date().toISOString() };
...
if (!STORES.length) {
  ...
  store.logoKey = "builtin"; saveStores();
...
function sessionCtx(req) {
  ...
  if (s && s.uid && s.sid) {
    ...
    if (user && store && user.active && user.storeId === store.id) return { user, store };
  }
  ...
  if (legacy && legacy.length === AUTH_TOKEN.length) {
    try {
      if (crypto.timingSafeEqual(Buffer.from(legacy), Buffer.from(AUTH_TOKEN))) {
        ...
        if (store && owner) return { user: owner, store };
      }
    } catch {}
  }
  return null;
...
app.post("/login", (req, res) => {
  ...
  const token = signSession({ uid: user.id, sid: store.id, iat: Date.now() });
  res.setHeader("Set-Cookie", `wtr_sess=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${180 * 24 * 3600}${process.env.RENDER ? "; Secure" : ""}`);
  res.redirect("/");
});
...
app.use((req, res, next) => {
  ...
  if ((req.path === "/" || req.path === "/appeal" || req.path === "/history" || req.path === "/resources" || req.path === "/console" || req.path === "/account") && req.method === "GET") {
...
app.get("/api/owner/stores", requireOwner, (_req, res) => {
  res.json({
    stores: STORES.map((s) => ({ id: s.id, name: s.name, city: s.city, slug: s.slug, active: s.active !== false, plan: s.plan, hasLogo: !!s.logoKey, users: USERS.filter((u) => u.storeId === s.id).length })),
    users: USERS.map((u) => ({ id: u.id, storeId: u.storeId, email: u.email, name: u.name, role: u.role, active: u.active !== false })),
  });
});
