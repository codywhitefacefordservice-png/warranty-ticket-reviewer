...
async function fetchProviderStatus(url) {
  // Returns ONLY a level. We deliberately do NOT pass through the provider's
  // own description text, because it can name the underlying vendor. All
  // user-facing wording is generated by us below, vendor-free.
  try {
    const ctl = new AbortController();
...
async function getSystemStatus() {
  ...
  // Map the (hidden) upstream providers onto plain-English capabilities.
  // Hosting: the user is connected, so the site itself is up. Only reflect a
  // hosting problem if the platform explicitly reports one; an unreachable feed
